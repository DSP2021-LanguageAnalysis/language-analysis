...
def display_count_chart(n_clicks, values0, values1, periods):
    ...
    if n_clicks is not None:
        ...
        df['YearGroup'] = pd.cut(df['Year'], bins=bins,include_lowest=True, labels=labels, precision=0)
        df['YearGroup'] = df['YearGroup'].astype("str")
        df = df.groupby(['ID', 'YearGroup', 'Tags', 'SenderSex']).mean().reset_index()

        fig = go.Figure()
        mask = df['Tags'].isin(values1)
        df = df[mask].groupby(['ID', 'YearGroup', 'SenderSex', 'WordCount']).sum().reset_index()
        df['LetterCount'] = 1
        df = df.groupby(['YearGroup', 'SenderSex']).sum().reset_index()
        ...
        fig= px.bar(
            ...
            # awnted to have the ID in the hoverdata, but then there is callback error (cant find ID?)
            #hover_data=['ID'],
            color='SenderSex',
            barmode='group',
            title='Letter Count Chart')
        fig.update_layout(yaxis_range=[0,510])
